...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to initiate account deletion' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message:
          'Deletion confirmation email sent. Please check your email to confirm account deletion.',
