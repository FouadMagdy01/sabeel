  ...
  try {
    ...
    if (normalizedInviteCode && !groupId) {
      ...
      if (findError) {
        ...
        return new Response(JSON.stringify({ error: 'Failed to lookup invite code' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (!group) {
        return new Response(JSON.stringify({ error: 'No group found with that invite code' }), {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (!targetGroupId) {
      return new Response(JSON.stringify({ error: 'Group ID or invite code required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!existingStreak) {
      const { error: streakInsertError } = await supabaseAdmin.from('group_streaks').insert({
        group_id: targetGroupId,
        user_id: user.id,
        current_streak: 0,
        longest_streak: 0,
