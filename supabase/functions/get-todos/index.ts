...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase
      .from("todos")
      .select("id, title, title_ar, description, description_ar, custom_points, points_config_key, todo_type, is_active")
      .eq("is_active", true);
    ...
    const pointsMap = new Map(
      (pointsConfig || []).map((p) => [p.key, p])
    );

    // Transform response based on language
    const transformedTodos = (todos || []).map((todo) => {
      const pointsConfigData = todo.points_config_key ? pointsMap.get(todo.points_config_key) : null;
