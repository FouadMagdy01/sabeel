...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (groupId) {
      ...
      // Record completion
      const { error: insertError } = await supabase.from('group_todo_completions').insert({
        group_id: groupId,
        user_id: userId,
        todo_id: todoId,
      ...
      console.log(
        `User ${userId} completed group todo ${todoId}, earned ${todo.custom_points} points`
      );
      ...
      const { error: insertError } = await supabase.from('user_todo_completions').insert({
        user_id: userId,
        todo_id: todoId,
