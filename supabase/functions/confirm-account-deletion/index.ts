...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (otpError || !otpRecord) {
      return new Response(JSON.stringify({ error: 'Invalid or expired deletion token' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await supabaseAdmin.from('group_todo_completions').delete().eq('user_id', userId);

    // 2. Delete user's group streaks
    await supabaseAdmin.from('group_streaks').delete().eq('user_id', userId);
    ...
    await supabaseAdmin.from('user_todo_completions').delete().eq('user_id', userId);

    // 6. Delete user's personal todo completions
    await supabaseAdmin.from('user_personal_todo_completions').delete().eq('user_id', userId);

    // 7. Delete user's personal todos
    await supabaseAdmin.from('user_personal_todos').delete().eq('user_id', userId);

    // 8. Delete user's challenges
    await supabaseAdmin.from('user_challenges').delete().eq('user_id', userId);

    // 9. Delete user's favorite suras
    await supabaseAdmin.from('favorite_suras').delete().eq('user_id', userId);
    ...
    await supabaseAdmin.from('user_streaks').delete().eq('user_id', userId);
