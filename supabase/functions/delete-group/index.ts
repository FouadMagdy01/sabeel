...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (memberError || membership?.role !== 'admin') {
      ...
      return new Response(JSON.stringify({ error: 'Only group admins can delete the group' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    await supabaseAdmin.from('group_todo_completions').delete().eq('group_id', groupId);

    // Delete group todos
    await supabaseAdmin.from('group_todos').delete().eq('group_id', groupId);

    // Delete group streaks
    await supabaseAdmin.from('group_streaks').delete().eq('group_id', groupId);

    // Delete group points config
    await supabaseAdmin.from('group_points_config').delete().eq('group_id', groupId);
    ...
    return new Response(JSON.stringify({ success: true, message: 'Group deleted successfully' }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
