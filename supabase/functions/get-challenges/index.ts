...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase
      .from("challenges")
      .select("id, title, title_ar, description, description_ar, points, start_date, end_date, is_active");
    ...
    const transformedChallenges = currentChallenges.map((challenge) => ({
      ...
      description: lang === "ar" && challenge.description_ar ? challenge.description_ar : challenge.description,
