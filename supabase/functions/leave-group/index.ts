  ...
  try {
    ...
    if (membership?.is_admin && admins && admins.length === 1) {
      return new Response(
        JSON.stringify({
          error:
            'Cannot leave group as the only admin. Transfer admin rights first or delete the group.',
        }),
    ...
    return new Response(JSON.stringify({ success: true, message: 'Successfully left group' }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
